...
====
* Compact single-header format: around 5000 lines with no dependencies.
...
Usually, LEAF error handlers are selected based on the type of the arguments they take and the type of the available error objects. When an error handler takes a predicate type as an argument, the <<handler_selection_procedure,handler selection procedure>> is able to also consider the _value_ of the available error objects.
...
A single-header option is available; please `#include <boost/leaf/all.hpp>`.

There is also <<tutorial>> and a reference section split in five parts, the contents of each part organized alphabetically: <<functions,Functions>>, <<types,Types>>, <<predicates,Predicates>>, <<macros,Macros>>, <<traits,Traits>>.
...
LEAF supports error-reporting via a (possibly external) `result<T>` type or by throwing exceptions. Functions that throw exceptions or use exception handling are defined in separate headers, so that client code that does not use exceptions is not coupled with them.
...
. Invoke the passed `try_block` function;
. If the `try_block` fails, they invoke the first of the specified error handlers that LEAF is able to supply with arguments using the available (in `ctx`) error objects.

In addition, the `context` template provides a lower-level error handling member function, <<context::handle_error>>, which selects an error handler based on available error objects in `*this`, associated with the supplied <<error_id>>. This function is designed to be called after the caller has detected a failure; it does not use a `result` type and can not deal with exceptions. Use one of the `try_` functions (above) for these cases.

[[context.hpp]]
==== `context.hpp`

This header defines the `context` template, which is used in error-handling scopes to provide storage for the error objects needed by user-defined error-handling functions. All communicated error objects are stored in a `std::tuple` internal to a `context` instance.
...
This header defines error handler predicates. See <<tutorial-predicates>>.
...
[.text-right]
Reference: <<match>> | <<match_value>> | <<match_member>> | <<if_not>> | <<category,`category`>> | <<condition,`condition`>>
====
...
[.text-right]
Reference: <<context.hpp>> | <<try_handle_all>> | <<try_handle_some>> | <<error_info>> | <<diagnostic_info>> | <<verbose_diagnostic_info>>
====
...
[.text-right]
Reference: <<handle_error.hpp>> | <<try_catch>> | <<catch_>>
====
...
+
* If `a~i~` is of a predicate type `Pred` (for which `<<is_predicate,is_predicate>><Pred>::value` is `true`), LEAF deduces:
+
[.text-center]
`using E = typename std::decay<decltype(std::declval<Pred>().matched)>::type;`
+
If an error object `e` of type `E`, associated with `err`, is currently stored in `ctx`, and if the expression `Pred::evaluate(e)` is `true`, `a~i~` is initialized with `Pred{e}`. Otherwise, the handler is dropped.
+
...
* `Pred` defines an accessible data member `matched`.
* `Pred` defines an accessible static member function `evaluate`, which returns a boolean type, and can be invoked with an object of type `E const &`, deduced as:
+
[.text-center]
`using E = typename std::decay<decltype(std::declval<Pred>().matched)>::type`.

When an error handler takes an argument of a predicate type `Pred`, the <<handler_selection_procedure,handler selection procedure>> will first ensure that an error object `e` of type `E` is available, and then if `Pred::evaluate(e)` returns `true`, the `Pred` argument is initialized with `Pred{e}` to invoke the error handler.
...
If the error handler is invoked, the `caught` member can be used to access the exception object.
...
leaf::try_catch(
  ...
  { <1>
    assert(dynamic_cast<ex1 const *>(&c.caught) || dynamic_cast<ex2 const *>(&c.caught));
    ....
  } );
----
...
LEAF requires {CPP}11 or newer compiler.

It is formatted as a Boost library, and as such it depends only on Boost Config and Boost MP11.



The header `leaf.hpp` is provided as a standalone single-header option. If that header is used,
...
Clone LEAF under your `boost/libs` directory and execute:
...
* `BOOST_LEAF_STANDALONE`: Defining this macro makes LEAF a standalone zero-dependency library (by default it uses `<boost/config.hpp>` and `<boost/mp11/algorithm.hpp>`).
+
[TIP]
--
The header `leaf.hpp` provides a single-header option which has `BOOST_LEAF_STANDALONE` defined by default.
--
+
