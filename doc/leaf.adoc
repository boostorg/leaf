...
Values of type `error_id` identify an error across the entire program. They can be copied, moved, assigned to, and compared to other `error_id` objects. They occupy as much memory, and are as efficient as an `int`.

The function <<success>> returns a special `error_id` value that does not represent any error.

To report an error, call the <<new_error>> function to obtain a new unique ID for the error.
...
Returns: :: An `error_id` value that indicates success. This value is guaranteed to be different from any value returned by <<new_error>>, <<next_error>> or <<last_error>>.
