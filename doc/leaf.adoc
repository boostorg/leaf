...
The `error` type derives publicly from `std::error_code`.  It does not add any data members, so objects of type `error` are as efficient as objects of type `std::error_code`.

Values of type `error` identify an error across the entire program. They can be copied, moved, assigned to, and compared to other `error` or `std::error_code` objects.

A default-initialized `error` object does not represent an error condition. It compares equal to any other default-initialized `error` object.

All other `error` objects use an `error_category` of unspecified type. Such errors should be handled using <<handle_all>> / <<handle_some>> or <<try_>>, which are able to recognize their special semantics, yet are able to deal with any other `std::error_code`.

To report an error, call the <<new_error>> function to obtain a new unique `error` value for the error.
...
namespace boost { namespace leaf {
  ...
  bool error_id::operator bool() const noexcept;

} }
----

Returns: :: If the `error_id` value was obtained by a call to <<success>>, returns false. Otherwise returns `true`.
...
namespace boost { namespace leaf {
  ...
  std::ostream & operator<<( std::ostream & os, error_id const & id )

} }
----

Effects: :: Prints an `unsigned int` value that uniquely identifies the value `id`.
...
include::{sourcedir}/synopses/error_code.adoc[]

This header supports interoperability between LEAF and APIs which communicate errors using `std::error_code`.

Typically, the `try_block` passed to <<handle_some>> or <<handle_all>> returns an instance of the <<result>> template, which holds an <<error_id>> necessary to match error handlers to available error objects.

The <<get_error_code>> function returns a special `std::error_code` value which can be passed to `handle_some` or `handle_all` as well to access available error objects. This enables users to communicate LEAF errors through any `std::error_code` interface.
...
Returns: :: If `id` represents success, returns a default-initialized `std::error_code`. Otherwise, the returned `std::error_code` will have an unspecified non-zero `.value()` and an unspecified `.category()`.
+
If the returned `std::error_code` is returned by a `try_handler` passed to <<handle_some>> or <<handle_all>>, the original `id` is recovered and handler matching proceeds as usual.

'''

[[get_error_id-error_code]]
...
Returns: ::
* If `ec` represents success, returns the value returned by <<success>>;
* Otherwise, if `ec` was not constructed using <<get_error_code>>, returns the value returned by <<next_error>>;
* Otherwise, returns the `id` originally passed to <<get_error_code>>.
...
In addition, `handle_some` is compatible with any other type for which an unqualified call to `get_error_id` can be bound. For example, LEAF provides <<error_code,helper functions>> that enable `handle_some` or <<handle_all>> to be used with `std::error_code` objects.

Requirements: ::

* The `try_block` must be a function callable with no arguments that returns a value `r` for which the expression `<<get_error_id-result,get_error_id>>(r)` is well  defined and returns an <<error_id>> object (note: `<<result,result>><T>` and `std::error_code` satisfy this requirement).
...
** may take a `<<failed,failed>><R>` argument, where `R` is the type returned by `try_block`;
...
Return Value: ::
If `<<get_error_id-result,get_error_id>>(r)`, where `r` is the object returned by the `try_block` function, indicates success, `handle_some` returns `r`.
+
Otherwise, `handle_some` considers each of the `handler...` functions, in order, until it finds one that matches the reported error. The first matching handler is invoked and its return value is forwarded to the caller of `handle_some`; if no match is found, `handle_some` returns `r`.
+
...
+
If `err` is the specific <<error_id>> obtained by `<<get_error_id-result,get_error_id>>(r)`, where `r` is the object returned by the `try_block`, each argument value `a`~i~ to be passed to the `handler` currently under consideration is produced as follows:
+
--
...
return leaf::handle_some(
  ...
  [ ]( leaf::failed<std::error_code> && r ) <2>
  {
    std::cerr << "std::error_code says \"" << r.value.message() << '"' << std::endl; <2>
    return r.value; <3>
  } );
----
<1> The `try_block` returns `std::error_code`, which...
<2> ...in case of failure will be passed to this handler, and then...
<3> ...it will be returned out of `handle_some`.
+
...
+
--
* If `T` is the type returned by the `TryBlock`, all `handler...` arguments must be functions that return `T` or a type that converts implicitly to `T` (note: `handle_some` handlers are required to return `result<T>`).
--
...
* Otherwise, it attempts to match the <<error_id>> communicated with the caught exception with a `handler`. If that succeeds, `try_` forwards the return value from the matched `handler` to the caller;
