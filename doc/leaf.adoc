...
=== Hierarchy-Free Classification of Errors
...
TIP: This programming technique works similarly when errors are communicated by a `result` type, rather than by throwing exceptions.
...
Requires: :: Each of `item...` must be no-throw movable.
...
Requires: :: Each `Item` type must be no-throw movable.
