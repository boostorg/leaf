...
:stylesheet: zajo.css
= LEAF
Lightweight Error Augmentation Framework for C++11
:toclevels: 3
:toc: left
:toc-title:
...
[.text-right]
https://github.com/zajo/leaf[GitHub] | <<introduction,Introduction>> | <<tutorial>> | <<rationale,Design Rationale>>
====
...
While it is possible to instantiate the `context` class template directly with a list of E-types, this is prone to errors, because when a function attempts to report an E-object, unless a scope up the call chain contains a local `context` instantiated with that E-type, the E-object is discarded.

This behavior is made perfectly safe by automatically deducing the `E...` types used to instantiate the `context` template, from the list of handlers that actually recognize and recover from various error conditions. Example:
...
"To load" an E-object is to associate it with a particular <<error_id>>, making it available to functions that handle the error.
...
As is always the case with LEAF, the accumulation only takes place if a handler passed to <<try_handle_some>>, <<try_handle_all>> or <<try_catch>> takes an argument of type `e_relevant_file_names`; otherwise the active `context` would not provide storage for this type and the corresponding accumulation code would not execute.

In other words, the accumulation of `e_relevant_file_names` will only occur if a caller actually uses that information.
...
<<tutorial_preload,Using `preload`>> we can  solve a very similar problem without a wrapper function, but that technique does not work for `e_errno` because `<<preload,preload>>` would capture `errno` before a `fread` call was attempted, at which point `errno` is probably `0` -- or, worse, leftover from a previous I/O failure.
...
[[tutorial_deferred_accumulate]]

=== Deferred `accumulate`

Let's say we want to build a record of file locations a given error passes through on its way to being handled. We couldn't do it with `preload`, because in this case we need to accumulate information, rather than store it.
...
NOTE: The helper function `remote_handle_all`, as well as its alternatives `remote_handle_some` and `remote_handle_exception` have no purpose other than to enable capturing of remote handlers; do not call them in any other case.

With this in place, reusing these so-called remote handlers is a simple matter of calling `leaf::remote_try_handle_all` instead of `leaf::try_handle_all`:
...
With LEAF, <<e_objects>> use automatic storage duration, stored in an instance of the <<context>> template in the scope of e.g. <<try_handle_some>>, <<try_handle_all>> or <<try_catch>> functions. When using concurrency, we need a mechanism to capture E-objects in one thread, then use them to handle errors in another thread.
...
Because the task will run asynchronously, in case of a failure we need it to capture the relevant E-objects but not handle errors. To this end, in the main thread we first create a remote handler which we will later use to handle errors from each completed asynchronous task (see <<tutorial_remote_handlers>>):
...
[.text-right]
<<make_shared_context>> | <<capture>> | <<result>>
...
Just like we saw in <<tutorial_async-result>>, first we will create a remote handler:
...
[[tutorial_disparate_error_types]]
...
[[tutorial_preload_in_c_callbacks]]
...
This header defines functions for automatic inclusion of E-objects with any error exiting the scope in whic they are invoked. See <<tutorial_preload>>.
...
The `context` template is used in error-handling scopes to provide storage for the error objects needed by user-defined error-handling functions, and to handle errors.

`context` and error-handling headers are designed to minimize coupling:
...
* Functions that take a `TryBlock` function containing a set of operations to try and then handle errors, contain the word `try_` in their name. All other functions work with an error or an exception from an operation that has already failed.
* Functions that *do not* use the `remote_` prefix take a list of error handlers; functions that *do*, take a single error-handling function, which internally captures the list of error handlers. See <<tutorial_remote_handlers>>.
...
* An `pass:[_all]` or a `pass:[_some]` function does not catch or handle exceptions unless  at least one of the user-supplied handlers uses the <<catch_>> template. All other error-handling functions catch and can handle exceptions.
* Error-handling functions that are members of the `context` template match handlers with error objects stored in `*this`:
+
.Error-handling members of the context template:
[cols="20%,40%,40%", stripes=none]
|====
| | Attempts a set of operations, then handles errors: |  Handles errors from an operation that has already failed:
| Handles errors communicated by a `result<T>` type:
| <<try_handle_some>>pass:[*] +
<<try_handle_all>>pass:[*] +
<<remote_try_handle_some>>pass:[*] +
<<remote_try_handle_all>>pass:[*]
|  <<handle_some>> +
<<handle_all>> +
<<remote_handle_some>> +
<<remote_handle_all>>
...
|====
+
* Namespace-scope error-handling functions are implemented in terms of a `context` object (created in their scope) of type that is deduced automatically from the user-supplied handlers:
+
.Namespace-scope error-handling functions:
[cols="20%,40%", stripes=none, width=60%]
|====
| | Attempts a set of operations, then handles errors:
| Handles errors communicated by a `result<T>` type:
| <<try_handle_some>>pass:[*] +
<<try_handle_all>>pass:[*] +
<<remote_try_handle_some>>pass:[*] +
...
|====
+
NOTE: pass:[*] These functions will catch and handle exceptions iff at least one of the user-supplied handlers uses <<catch_>>; otherwise they will not (dispatched statically).
...
namespace boost { namespace leaf {
  ...
  public:
    ...
    handle_all( R const &, H && ... ) const noexcept;

    template <class R, class RemoteH>
    typename std::decay<decltype(std::declval<R>().value())>::type
    remote_handle_all( R const &, RemoteH && ) const noexcept;
...
[.text-right]
<<context>> | <<context_type_from_remote_handler>> | <<make_context>> | <<make_shared_context>> | <<allocate_shared_context>> | <<polymorphic_context>>
...
[.text-right]
<<context.hpp>> | <<try_handle_all>> | <<try_handle_some>> | <<remote_try_handle_all>> | <<remote_try_handle_some>> | <<match>> | <<condition>> | <<error_info>> | <<diagnostic_info>> | <<verbose_diagnostic_info>>
...
[.text-right]
<<handle_error.hpp>> | <<try_catch>> | <<remote_try_catch>> | <<catch_>
...
This function works similarly to <<remote_try_handle_some>>, except that the captured handlers are to be used with `remote_try_catch` rather than `remote_try_handle_some`, therefore when capturing the handlers we call the helper function `remote_handle_exception` rather than `remote_handle_some`:
...
To use the captured `remote_handlers`, we call `remote_try_catch` rather than `try_catch` (the former requires the handlers to be provided in a handler pack):
...
This function works similarly to <<remote_try_handle_some>>. One difference is that, like <<try_handle_all>>, it is required (enforced at compile time) to be able to find a matching handler for the error. Therefore, the handlers captured in `remote_handler` must include a handler that matches any error, and may not throw:
...
TIP: While `try_remote_handle_some` is useful just for being able to capture a set of handlers to be used at a later time, its typical use is to allow for automatic `context` deduction when errors are transported to a different thread; see <<capture>>.
...
The `try_catch` function works similarly to <<try_handle_some>>, except that it does not use or understand the semantics of `result<T>` types. Instead, it assumes that the `try_block` throws to indicate a failure, in which case `try_catch` will attempt to find a matching handler among `h...`.
...
----
[.text-right]
<<context::context>> | <<context::activate>> | <<context::deactivate>> | <<context::thread_id>> | <<context::print>> | <<context::try_handle_all>> | <<context::remote_try_handle_all>> | <<context::try_handle_some>> | <<context::remote_try_handle_some>> | <<context::handle_all>> | <<context::remote_handle_all>> | <<context::handle_some>> | <<context::remote_handle_some>> | <<context::handle_current_exception>> | <<context::remote_handle_current_exception>> | <<context::handle_exception>> | <<context::remote_handle_exception>>
...
namespace boost { namespace leaf {
  ...
  template <class R, class... H>
  typename std::decay<decltype(std::declval<R>().value())>::type
  context<E...>::handle_all( R const & r, H && ... h ) const noexcept;
...
NOTE: Handlers passed to `handle_all` may be matched to the current exception using <<catch_>>. In this case please `#include <boost/leaf/handle_exception.hpp>`.
...
The requirements for the passed `h...` are same as <<context::handle_some>>, but the return type of each `h` must be suitable for initializing the specified return value `R`.

Handler Matching Procedure: ::

Because each E-object stored in `*this` is uniquely associated with a specific <<error_id>>, `handle_current_exception` needs to extract an `error_id` value from the current exception in order to access any currently stored E-object.
+
...
** may take arguments, either by value or by `const &`, of the predicate type `<<catch_,catch_>><Ex...>`, where `Ex` derives from `std::exception` (please `#include <boost/leaf/handle_exception.hpp>`);
...
NOTE: `handle_some` is exception-neutral: it does not throw exceptions, however the user-upplied handlers are permitted throw.
...
+
** If `A`~i~ is of the predicate type `<<match,match>><E>`, if an object of type `E`, associated with `err`, is currently stored in `*this`, `a`~i~ is initialized with a reference to the stored object; otherwise the handler is dropped. The handler is also dropped if the expression `a`~i~`()` evaluates to `false`.
+
.Example:
[source,c++]
----
...
namespace boost { namespace leaf {
  ...
  template <class R, class RemoteH>
  typename std::decay<decltype(std::declval<R>().value())>::type
  context<E...>::remote_handle_all( R const & r, RemoteH && h ) const noexcept;
...
----
+
NOTE: The namespace-scope `remote_handle_all` invoked from the user-defined `handle_error` is a helper function which serves no purpose other than to return a function compatible with `context::remote_handle_all`.
+
...
----
+
NOTE: The namespace-scope `remote_handle_exception` invoked from the user-defined `handle_error` is a helper function which serves no purpose other than to return a function compatible with `context::remote_handle_current_exception`.
+
...
----
+
NOTE: The namespace-scope `remote_handle_exception` invoked from the user-defined `handle_error` is a helper function which serves no purpose other than to return a function compatible with `context::remote_handle_exception`.
+
...
----
+
NOTE: The namespace-scope `remote_handle_some` invoked from the user-defined `handle_error` is a helper function which serves no purpose other than to return a function compatible with `context::remote_handle_some`.
+
...
namespace boost { namespace leaf {
  ...
  template <class TryBlock, class RemoteH>
  typename std::decay<decltype(std::declval<TryBlock>()().value())>::type
  context<E...>::remote_try_handle_all( TryBlock && try_block, RemoteH && h ) noexcept
  {
    context_activator active_context(*this,
      context_activator::on_deactivation::do_not_propagate);
...
[.text-right]
<<context_activator>> | <<context::remote_handle_all>>

'''

[[context::remote_try_handle_some]]
...
namespace boost { namespace leaf {
  ...
  template <class TryBlock, class RemoteH>
  typename std::decay<decltype(std::declval<TryBlock>()())>::type
  context<E...>::remote_try_handle_some( TryBlock && try_block, RemoteH && h )
  {
    context_activator active_context(*this,
      context_activator::on_deactivation::propagate_if_uncaught_exception);
...
[.text-right]
<<context_activator>> | <<context::remote_handle_some>>
...
namespace boost { namespace leaf {
  ...
  template <class TryBlock, class... H>
  typename std::decay<decltype(std::declval<TryBlock>()().value())>::type
  context<E...>::try_handle_all( TryBlock && try_block, H && ... h ) noexcept
  {
    context_activator active_context(*this,
      context_activator::on_deactivation::do_not_propagate);
...
[.text-right]
<<context_activator>> | <<context::handle_all>>

'''

[[context::try_handle_some]]
...
namespace boost { namespace leaf {
  ...
  template <class TryBlock, class... H>
  typename std::decay<decltype(std::declval<TryBlock>()())>::type
  context<E...>::try_handle_some( TryBlock && try_block, H && ... h )
  {
    context_activator active_context(*this,
      context_activator::on_deactivation::propagate_if_uncaught_exception);
...
[.text-right]
<<context_activator>> | <<context::handle_some>>
