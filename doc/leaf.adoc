...
Error-reporting function: :: A function that detects and reports a new failure. Usually such functions call `new_error` to generate a new `error_id` for each error condition they encounter; typically, at least one E-object is associated with the new `error_id` at this point.
...
A possible workaround is to call <<new_error>> some time after the original exception was handled, if possible (LEAF does call `new_error` when it handles an exception that does not carry an `error_id`).
--
...
This section of the synopsis lists header files needed to report errors or to throw exceptions.
...
This header defines functions for automatic inclusion of E-objects with any error exiting the scope in whic they are invoked. See <<tutorial-preload>>, <<tutorial-defer>>, <<tutorial-accumulate>>.
...
CAUTION: Be extra careful, since <<tutorial-accumulation>> naturally may need to allocate memory. In this case consider using `<<error_id::accumulate,error_id::accumulate>>` instead, invoked *not* from a destructor, in which case throwing exceptions would be permitted.
...
This function works similarly to <<remote_try_handle_some>>, but handles exceptions rather than a `result<T>` result. Here is an example of how remote handlers should be captured so the captured function is compatible with  `remote_try_handle_some`:
...
This function works the same way as <<try_handle_some>>, but rather than taking a parameter pack with handlers, it takes a single lambda that captures the handlers, for example:
...
The `try_handle_all` function creates a local <<context>> object of type that is automatically deduced from the passed handler list `h...`, which guarantees that it is able to store all of the types required to handle errors; then it calls the member function <<context::try_handle_all>>:
...
try_handle_all( TryBlock && try_block, H && ... h ) noexcept
{
  ...
  return ctx.try_handle_all(std::forward<TryBlock>(try_block), std::forward<H>(h)...);
}
----
...
The `try_handle_some` function creates a local <<context>> object of type that is automatically deduced from the passed handler list `h...`, which guarantees that it is able to store all of the types required to handle errors; then it calls the member function <<context::try_handle_some>>:
...
try_handle_some( TryBlock && try_block, H && ... h ) noexcept
{
  ...
  return ctx.try_handle_some(std::forward<TryBlock>(try_block), std::forward<H>(h)...);
}
----
...
* Error state, in which case it contains an object of type <<error_id>>, and calling `<<result::value,value>>`/`<<result::value,operator*>>` throws `leaf::bad_result`.
