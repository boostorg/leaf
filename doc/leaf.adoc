...
TIP: To avoid ambiguities, whenever possible, use the <<exception>> function template when throwing exceptions to ensure that the exception object transports a unique `error_id`; better yet, use the <<BOOST_LEAF_THROW>> macro, which in addition will capture `pass:[__FILE__]` and `pass:[__LINE__]`.
...
Error objects are stored in an instance of the <<context>> class template in the scope of e.g. <<try_handle_some>>, <<try_handle_all>> or <<try_catch>> functions, using automatic storage duration. When using concurrency, we need a mechanism to collect error objects in one thread, then use them to handle errors in another thread.
...
Because the task will run asynchronously, in case of a failure we need it to capture the relevant error objects but not handle errors. To this end, in the main thread we first create a remote handler which we will later use to handle errors from each completed asynchronous task (see <<tutorial-remote_handlers>>):
...
Why did we start with this step? Because we need to create a <<context>> object to collect the error objects we need. We could just instantiate the `context` template with `E1`, `E2` and `E3`, but that would be prone to errors, since it could get out of sync with the handlers we use. Thankfully LEAF can deduce the types we need automatically from the remote handler we created. To create our `context` object, we just call <<make_shared_context>>:
...
[.text-right]
<<remote_try_handle_some>> | <<result>> | <<BOOST_LEAF_AUTO>> | <<error_info>>

The reason this works is that in case it communicates a failure, `leaf::result<T>` is able to hold a `shared_ptr<polymorphic_context>` object. That is why earlier instead of calling `task()` directly, we called `leaf::capture`: it calls the passed function, and in case it fails it stores the `shared_ptr<polymorphic_context>` we created in the returned `result<T>`, which now doesn't just communicate the fact that an error has occurred, but also holds the `context` object that `remote_try_handle_some` needs in order to supply a matching handler with arguments.
...
=== Classifying Errors Without Exception Type Hierarchy

Traditionally, exception types are organized in a hierarchy in order to classify failures:

[source,c++]
----
struct read_error      : virtual std::exception { };
struct file_error      : virtual std::exception { };
struct file_read_error : virtual read_error, virtual file_error { };
----
...
<2> Handle read errors that are not file errors.

LEAF supports a style of error handling where all exceptions thrown are of type `std::exception`. In that case, instead of the hierarchy above, we could simply define:
...
[source,c++]
----
throw leaf::exception(file_error{}, read_error{}); <1>
----
[.text-right]
<<exception>>

<1> In this invocation we are not passing an exception object to `leaf::exception`. In this case it returns an object of unspecified type that derives from `std::exception`.
...
TIP: We don't bother to use <<catch_>>, because `try_catch` always catches exceptions, and we've chosen to not dispatch based on the exception type.

This style enables us to use `on_error` to add further classification dynamically, after the initial reporting of the error, as it bubbles up the call stack. For example;

[source,c++]
----
config read_config(....);
{
  auto load = leaf::on_error(read_config_error{}); <1>
...
<1> Mark *any* failure that occurs inside `read_config` as `read_config_error`.
<2> Call functions which throw on error.
...
<1> Call more functions which may fail.
<2> Handle `read_config` failures due to a json parse error.
<3> Handle any other `read_config` failure.

TIP: This programming technique works similarly when errors are communicated by a `result` type, rather than by throwing exceptions; see below.

'''

=== Classifying Errors Without Enums

It is common for a library to define an `enum` that lists all possible error codes that any of the library functions may report. The benefit of this approach is that the list is complete and usually contains comments, so we know where to go for reference.

The disadvantage of such flat lists is that they do not support handling a whole class of errors. Consider this error handler lambda from the <<introduction-result,introduction section>>:
...
It will get called if the value of the `error_code` enum communicated with the error is one of `input_file_size_error`, `input_file_read_error` or `input_eof_error`. In short, the idea is to handle an input error.

But what if later we add support for detecting and reporting a new type of input error, e.g. `input_permissions_error`? We would add that to the `error_code` enum. And this would break our error handling.

LEAF supports an error handling model where the different error conditions are represented by individual static types, which act as a simple tagging system. If we go back to the introduction section, instead of defining:
...
With this in place, `file_read` can be implemented like this:
...
<1> This error is classified as both `input_error` and `read_error`.
<2> This error is classified as both `input_error` and `eof_error`.
...
#define BOOST_LEAF_THROW(...) ....
----

[.text-right]
<<exception>> | <<BOOST_LEAF_EXCEPTION>> | <<BOOST_LEAF_THROW>>
...
| [`<<remote_try_handle_all,remote_>>`]<<try_handle_all>> | Yes | *
| [`<<remote_try_handle_some,remote_>>`]<<try_handle_some>> | Yes | *
| [`<<remote_try_catch,remote_>>`]<<try_catch>> | No | Yes
|====

[.text-right]
pass:[*] Handles exceptions iff at least one of the supplied handlers uses <<catch_>> +
...
This function works similarly to <<remote_try_handle_some>>, but it is required to handle any error (enforced at compile-time). Therefore, the captured `remote_handlers` must include a handler that matches any error:
...
* In addition, it requires the passed handler pack `h...` to be able to handle any error, which is enforced at compile time, and
...
** may take any error objects, but only by value or by `const &`, or as a `const *`;
...
+
If `err` is the `error_id` obtained from `r.error()`, each argument value `a~i~` to be passed to the handler currently under consideration is produced as follows:
+
--
* If `a~i~` is taken as `A~i~` `const &` or by value:
** If an error object of type `A~i~`, associated with `err`, is currently stored in `ctx`, `a~i~` is initialized with a reference to the stored object; otherwise the handler is dropped.
+
.Example:
[source,c++]
----
...
----
[.text-right]
<<context::context>> | <<context::activate>> | <<context::deactivate>> | <<context::is_active>> | <<context::propagate>> | <<context::print>> | <<context::handle_error>> | <<context::remote_handle_error>> | <<context_type_from_remote_handler>>
...
While error handling typically uses <<try_handle_some>>, <<try_handle_all>> or <<try_catch>>, it is also possible to handle errors by calling the member functions <<context::handle_error>> and <<context::remote_handle_error>>. They take an <<error_id>>, and attempt to match an error handler with error objects stored in `*this`.
...
The <<BOOST_LEAF_NEW_ERROR>>, <<BOOST_LEAF_EXCEPTION>> and <<BOOST_LEAF_THROW>> macros capture `pass:[__FILE__]`, `pass:[__LINE__]` and `pass:[__FUNCTION__]` into a `e_source_location` object.
...
Effects: :: `BOOST_LEAF_NEW_ERROR(e...)` is equivalent to `leaf::<<new_error,new_error>>(e...)`, except the current source location is automatically passed to `new_error` in a `<<common,e_source_location>>` object (in addition to all `e...` objects).
...
.#include <boost/leaf/exception.hpp>
----
#define BOOST_LEAF_THROW(...) throw BOOST_LEAF_EXCEPTION(__VA_ARGS__)
----
