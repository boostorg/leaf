...
LEAF provides a very lightweight error-handling interface, which is nevertheless capable of transporting arbitrary payloads -- without allocating memory dynamically. It works equally well with or without exception handling; indeed, the two can be mixed seamlessly in the same program if necessary.

The library has a strong bias towards the common case where callers of functions which may fail check for success and forward errors up the call chain but do not handle them. In this case, only a simple success-or-failure discriminant is transported. Actual error objects are delivered directly to the correct error-handling scope, skipping the intermediate check-only frames altogether.
...
If `fopen` succeeds, we return a `shared_ptr` which will automatically call `fclose` as needed. If `fopen` fails, we report an error by calling `new_error`, which takes any number of error objects to associate with the error. In this case we pass the system `errno` (LEAF defines `struct e_errno {int value;}`), and our own error code value, `open_error`.
...
int main( int argc, char const * argv[] )
{
  return leaf::try_handle_all(
    ...
    .... // The list of error handlers goes here (we saw that earlier).
...
<5> This concludes the `try_handle_all` arguments -- as well as our program!
...
TIP: The complete program from this tutorial is available https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result.cpp?ts=4[here]. This https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_eh.cpp?ts=4[other] version of the same program uses exception handling to report errors (see <<introduction-eh,below>>).
...
<4> This concludes the `try_catch` arguments -- as well as our program!
...
int main( int argc, char const * argv[] )
{
  return leaf::try_catch(
    [&]
    {
      // The TryBlock goes here (previous listing).
    },
...
TIP: The complete program from this tutorial is available https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_eh.cpp?ts=4[here]. This https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result.cpp?ts=4[other] version of the same program does not use exception handling to report errors (see the <<introduction-result,previous introduction>>).
...
+
In particular, the `E` type used to instantiate the `match` template may be an instance of the <<condition>> class template, which is used to match enumerated values of some type `En` for which either of  `std::is_error_condition_enum<En>::value` or `std::is_error_code_enum<En>::value` is true:
+
.Example:
[source,c++]
----
...
+
<1> This handler is selected if the error includes an object of type `std::error_code` that compares equal to `condx::x11`.
...
namespace boost { namespace leaf {
  ...
  struct condition
  {
    ...
    static_assert(std::is_error_condition_enum<enum_type>::value || std::is_error_code_enum<enum_type>::value);
  };
...
[NOTE]
--
The `condition` class template is a workaround for {CPP}11 (compatible with newer {CPP} versions). If you can use {CPP}17 and newer, instead of:

[source,c++]
----
[]( leaf::match<leaf::condition<my_enum>, my_enum::v1, my_enum::v2, ...> )
{
}
----
...
[source,c++]
----
[]( leaf::match<std::error_code, my_enum::v1, my_enum::v2, ...> )
{
}
----
--

The `condition` template is useful only as argument to the <<match>> template, to match a received `std::error_code` to a list of values of an error condition enum.
