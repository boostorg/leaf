...
Returns: :: If `r` indicates success, or if no matching handler could be found, returns `r`. Otherwise returns the value returned by the matching handler from the list of handlers used in the handler capture.

NOTE: Because LEAF does not invoke the user-defined `handle_error` function directly, it can take any number of arguments in addition to the required argument of type `leaf::error_info const &`.
