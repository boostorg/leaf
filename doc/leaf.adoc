...
Error-reprorting function: :: A function that detects and reports a new failure. Usually such functions call `new_error` to generate a new unique identifier for each error condition they encounter; typically. at least one E-object is associated with the new `error_id` at this point.
...
[.text-right]
<<handle_all>> | <<handle_some>> | <<try_handle_all>> | <<try_handle_some>> | <<remote_try_handle_all>> | <<remote_try_handle_some>> | <<remote_handle_all>> | <<remote_handle_some>> | <<match>> | <<condition>> | <<error_info>> | <<diagnostic_info>> | <<verbose_diagnostic_info>>
...
[.text-right]
<<handle_current_exception>> | <<handle_exception>> | <<try_catch>> | <<remote_try_catch>> | <<remote_catch>> | <<catch_> | <<get_error_id>>
...
[.text-right]
<<polymorphic_context>> | <<context>> | <<context_type_from_remote_handler>> | <<make_context>> | <<make_shared_context>> | <<allocate_shared_context>>
...
Effects: :: Each of the `e...` objects is <<loading,loaded>> and uniquely associated with `*this`.
...
The `error_id` member function returns the program-wide unique <<error_id>> of the error.

The `error_code` member function returns a `std::error_code`, if available, or `this->error_id()` if it is not (note that `error_id` derives from `std::error_code`).

The `exception_caught` member function returns `true` if the handler that received `*this` is being invoked by <<try_catch>>, `false` if invoked by <<try_handle_some>> or <<try_handle_all>>.
...
The `operator<<` overload prints diagnostic information about each E-object currently stored in any active <<try_handle_some>>, <<try_handle_all>> or <<try_catch>> scope, but only if it is associated with the <<error_id>> returned by `error()`.
...
The message printed by `operator<<` includes the message printed by `error_info`, followed by basic information about E-objects that were communicated to LEAF (to be associated with the error) for which there was no storage available in any active <<try_handle_some>>, <<try_handle_all>> or <<try_catch>> scope (these E-objects were discarded by LEAF, because no handler needed them).
...
Handlers passed to <<try_handle_some>> or <<try_handle_all>> may take an argument of type `failed<R>`, where `R` is the type returned by the `try_block` passed to <<try_handle_some>> or <<try_handle_all>>, if they need to use the returned object (which indicated a failure).
...
* If `*this` is in <<result,error-capture state>>, all captured E-objects are <<loading,loaded>>, `*this` is converted to error state, and then
* If `*this` is in <<result,error state>>, returns `err.<<load,load>>(std::forward<E>(e...))`, where `err` is the <<error_id>> stored in `*this`.
...
namespace boost { namespace leaf {
  ...
  try_handle_some( TryBlock && try_block, Handler && ... handler );

} }
----

The `try_handle_some` function template can be used to recognize and handle some errors, forwarding any unrecognized errors to the caller.
...
When `try_handle_some` is invoked, LEAF reserves space inside of the `try_handle_some` scope (using automatic storage duration) to store the E-objects it would pass to the appropriate `handler` in case the `try_block` indicates an error.
+
The list of E-types that `try_handle_some` needs to store internally is deduced automatically, using the following steps, in order:
+
--
...
--
+
Next, `try_handle_some` calls the `try_block` function, which generally would call other functions as needed. When any function needs to report an error, it passes one or more E-objects to LEAF. For example, a function that returns R can report an error with a call to <<new_error>> in a `return` expression:
+
[source,c++]
----
if( file-open-fails )
...
----
+
As each E-type object is passed to LEAF, it is immediately moved to the `try_handle_some` (or <<try_handle_all>>, or <<try_catch>>) scope highest in the call stack that has available storage for that type, where it is uniquely associated with a specific <<error_id>> (in this case, the value returned by `new_error`). Objects of E-types for which none of these scopes have available storage are discarded.

Return Value: ::
If `bool(r)` is `true`, where `r` is the object returned by the `try_block` function, `try_handle_some` returns `r`.
+
Otherwise, `try_handle_some` considers each of the `handler...` functions, in order, until it finds one that matches the reported `r.error()`. The first matching handler is invoked and its return value is used to initialize the return value of `try_handle_some`, which can indicate success if the handler was able to handle the error, or failure if it was not.
+
If the value `try_handle_some` is returning indicates success, all stored E-objects are discarded. Otherwise, each stored E-object is moved to appropriate storage available in `try_handle_some` (or <<try_handle_all>>, or <<try_catch>>) scopes higher up the call stack (each moved object retains its unique association with its specific `error_id` value). E-objects for which such storage is not available are discarded.

Handler Matching Procedure: ::
+
A `handler` matches the reported failure iff `try_handle_some` is able to produce values to pass as its arguments. As soon as it is determined that an argument value can not be produced, the current `handler` is dropped and the matching procedure continues with the next `handler`, if any.
+
Because each error object stored in the `try_handle_some` scope is associated with a specific unique <<error_id>> value, the first thing `try_handle_some` must do is call `r.error()` to obtain an error ID key to match error objects with:
+
--
...
* Otherwise, `try_handle_some` proceeds in a mode where only the following types of handler arguments can match: `error_info`, `diagnostic_info`, `verbose_diagnostic_info`, or `failed<R>`, where `R` is the return type of the `try_block`.
--
+
...
** If an E-object of type `A`~i~, associated with `err`, is currently stored in the `try_handle_some` scope, `a`~i~ is initialized with a reference to the stored object; otherwise the handler is dropped.
+
.Example:
[source,c++]
----
...
<3> Return success out of `try_handle_some`.
+
** If `A`~i~ is of the predicate type `<<match,match>><E>`, if an object of type `E`, associated with `err`, is currently stored in the `try_handle_some` scope, `a`~i~ is initialized with a reference to the stored object; otherwise the handler is dropped. The handler is also dropped if the expression `a`~i~`()` evaluates to `false`.
+
.Example:
[source,c++]
----
...
+
* If `a`~i~ is of type `A`~i~ `const pass:[*]`, `try_handle_some` is always able to produce it: if an E-object of type `A`~i~, associated with `err`, is currently stored in the `try_handle_some` scope, `a`~i~ is initialized with the address of the stored object, otherwise it is initialized with `0`.
+
.Example:
[source,c++]
----
...
<3> Return the error, which will be returned out of `try_handle_some` (in a `result<void>`).
+
...
<3> Return the error, which will be returned out of `try_handle_some` (in a `result<void>`).
+
* If `a`~i~ is of type `<<failed,failed>><R>`, where `R` is the type returned by the `try_block`,  `try_handle_some` is always able to produce it.
+
.Example:
[source,c++]
----
...
<3> ...we return success out of `try_handle_some`.
+
* It is illegal to pass to `try_handle_some` a `handler` that takes any other argument types.
--

TIP: Because `try_handle_some` can always produce arguments of type `error_info const &`, `diagnostic_info const &`, `verbose_diagnostic_info const &` and `failed<R>`, a handler that only takes arguments of these types will match any error.
...
`try_handle_all` has the same requirements as <<try_handle_some>>, with the following change:
+
--
...
* Otherwise the value returned by the first matched (using the same matching procedure as <<try_handle_some>>) handler is used to initialize the return value (of type `T`) of `try_handle_all`.
...
NOTE: The `match` template is useful only as argument to a handler function passed to <<try_handle_some>> (or `try_handle_all` or `try_catch`).
...
The `capture_result` function can be used to transport E-objects <<loading,loaded>> when `f` is invoked in one thread, to a different thread which handles errors through <<try_handle_some>> or <<try_handle_all>>.
...
+
--
* Like <<try_handle_some>>, `fw` reserves storage in its scope for objects of the `E...` types, then
...
+
The returned `result<T>` can safely cross thread boundaries (presumably sent back to the main thread). When it is returned from the `try_block` function passed to <<try_handle_some>> or <<try_handle_all>>, its contents are <<loading,loaded>> (in that thread), and then error handling proceeds as usual.
...
`try_catch` has the same requirements as <<try_handle_some>>, with the following change:
+
--
...
`try_catch` works similarly to <<try_handle_some>> -- the difference is that `try_catch` catches exceptions thrown by the `try_block` function:

* If the `try_block` throws:
** `try_catch` attempts to match the <<error_id>> communicated with the caught exception with a `handler`. If that succeeds, `try_catch` forwards the return value from the matched `handler` to the caller;
** Otherwise, `try_catch` re-throws the caught exception.
* If the `try_block` returns a value `r`:
** if `r` is of a `result<T>` type (see <<result_requirements>>), `try_catch` proceeds the same as `try_handle_some`;
** otherwise it returns `r`.

Handler Matching Procedure: ::

Because each E-object stored in the `try_catch` scope (see <<try_handle_some>>) is uniquely associated with a specific <<error_id>>, `try_catch` needs to extract an `error_id` value from the caught exception in order to access any currently stored E-object (`try_handle_some` and `try_handle_all` do not have this challenge, since the <<error_id>> they need is readily available in the `<<result,result>><T>` object communicating the failure).
+
...
+
Similarly, if the exception object caught by `try_catch` does not transports an `error_id` value, E-objects are looked up using the `error_id` value returned by <<next_error>> just before `try_catch` goes through its `handler`-matching search. If imperfect, this approach provides the needed association.
+
NOTE: <<get_error_id-exception>> implements the procedure described above.
+
With the `error_id` value thus obtained, the `handler`-matching procedure works the same as in <<try_handle_some>>, with the following addition to the way each argument value `a`~i~ to be passed to the `handler` currently under consideration is produced:
...
----
<1> This handler matches if the exception caught by `try_catch` is either of type `exception1` or `exception2`.
...
NOTE: Please read the documentation for <<try_catch>> to understand why `get_error_id` is needed.
...
{
  if( condition )
    leaf::get_error_id(e).load( e_this{....}, e_that{....} );
  throw;
}
----
[.text-right]
<<get_error_id-exception>> | <<error_id::load>>
...
But if the caught exception doesn't  have a `leaf::error_id` subobject, `get_error_id` returns an unspecified `leaf::error_id`, which is temporarily associated with any and all current exceptions, until successfully handled by <<try_catch>>. While this association is imperfect (because it does not pertain to a specific exception object) it is the best that can be done in this case.
...
{
  if( condition )
    err.load( e_this{....}, e_that{....} );
  throw;
}
catch( std::exception const & e )
{
  if( condition )
    leaf::next_error().load( e_this{....}, e_that{....} );
  throw;
}
----
[.text-right]
<<error_id::load>> | <<next_error>>
--
