...
+
--
* If <<new_error>> was invoked (by the calling thread) since the object returned by `defer` was created, each of the stored `f...` is called, and each returned object is <<tutorial-loading,loaded>> and uniquely associated with  `<<last_error,last_error>>()`;
...
* If <<new_error>> was invoked (by the calling thread) since the object returned by `preload` was created, the stored `e...` objects are <<tutorial-loading,loaded>> and become uniquely associated with `<<last_error,last_error>>()`;
...
try_handle_some( TryBlock && try_block, H && ... h ) noexcept
{
 ...
 on_deactivation::propagate_if_uncaught_exception);
