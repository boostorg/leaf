...
** may take arguments, by value, of any predicate type: <<catch_>>, <<match>>, <<match_value>>, <<match_member>>, <<if_not>>;
...
+
* If `a~i~` is a predicate type `Pred`, LEAF deduces the type `E` based on its type (usually this is simply the first argument to the predicate template). If an error object `e` of type `E`, associated with `err`, is currently stored in `ctx`, and if the predicate evaluates to `true`, `a~i~` is initialized with `Pred{e}`. Otherwise, the handler is dropped.
+
