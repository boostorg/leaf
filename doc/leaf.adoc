...
[cols="<,^,^,^,^", stripes=none]
|====
|  <| Attempts a set of operations, then handles errors <| Handles errors from a failed operation <| Handles `result<T>` errors <| Handles exceptions
5+^| Namespace-scope error-handling functions use an internal `context<E...>` object, deducing the `E...` types automatically from the arguments of the supplied handlers:
...
5+^| Members of the `context` template match handlers with error objects stored in `*this`:
| [`<<context::remote_try_handle_all,remote_>>`]<<context::try_handle_all>> | ✅ | ❌| ✅ | ✱
| [`<<context::remote_try_handle_some,remote_>>`]<<context::try_handle_some>> | ✅ | ❌| ✅ | ✱
...
(Please `#include <boost/leaf/handle_exception.hpp>`)
