...
[source,c++]
----
[]( leaf::match<std::error_code, libfoo_error::e1, libfoo_error::e2> )
{
  // We got either a libfoo_error::e1 or a libfoo_error::e2
}
----
...
Let's say that our program uses two libraries, `libfoo` and `libbar`, each communicating failures in terms of `std::error_code` with different error categories. Let's say `libbar_error` looks like this:
...
<2> Define the `std::error_category` for `my_error_condition` `std::error_condition` objects.
<3> Here we specify that any of `libfoo:error::e1`, `libbar_error::e3` and `libbar_error::e4` are logically equivalent to `my_error_condition::c1`...
<4> ...and that any of `libfoo:error::e2`, `libbar_error::e1` and `libbar_error::e2` are logically equivalent to `my_error_condition::c2`.
...
Now, if we have a `std::error_code` object `ec`, we can check if it is equivalent to `my_error_condition::c1` like so:
...
In terms of a LEAF error handler:

[source,c++]
----
[]( leaf::match<std::error_code, my_error_condition::c1> )
{
  // We have a c1 in our hands
}
----
