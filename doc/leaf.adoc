...
int main( int argc, char const * argv[] )
{
  return leaf::try_handle_all(
    ...
    [](leaf::match<error_code, open_error>, <1>
    ...
    [](leaf::match<error_code, open_error>, <2>
    ...
    [](leaf::match<error_code, size_error, read_error, eof_error>, <3>
        leaf::e_errno const * errn,
        leaf::e_file_name const & fn)
    {
    ...
    [](leaf::match<error_code, output_error>, <4>
        leaf::e_errno const & errn)
    {
...
int main( int argc, char const * argv[] )
{
  return leaf::try_catch(
    ...
    [](leaf::catch_<std::ostream::failure>, <5>
        leaf::e_errno const & errn )
    {
      std::cerr << "Output error, errno=" << errn << std::endl;
      return 4;
    ...
    [](leaf::catch_<bad_command_line>) <6>
    {
      std::cout << "Bad command line argument" << std::endl;
      return 5;
    },
...
<1> This is the `TryBlock` from the previous listing; if it throws, `try_catch` will catch the exception, then consider the error handlers that follow, in order, and it will call the first one that can deal with the error:
...
<6> This handler will be called if a `bad_command_line` exception was caught.

<7> If `try_catch` fails to find an appropriate handler, it will re-throw the exception. But this is the `main` function which should handle all exceptions, so this last handler matches any error and prints diagnostic information, to help debug logic errors.
...
----
....
[](leaf::match<error_code, size_error, read_error, eof_error>,
    leaf::e_errno const * errn,
    leaf::e_file_name const & fn)
{
  std::cerr << "Failed to access " << fn.value;
