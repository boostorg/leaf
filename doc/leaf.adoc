...
Requirements: :: `TryBlock` must return a `result<T>` object (see <<is_result_type>>). It is valid for the `TryBlock` to return `leaf::<<result,result>><T>`, however this is not a requirement.
