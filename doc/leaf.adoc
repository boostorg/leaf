...
[.text-right]
https://github.com/zajo/leaf[GitHub] | link:./leaf.pdf[PDF]
endif::[]
...
We'll see later just what kind of a `TryBlock` will our `main` function pass to `try_handle_all`, but first, let's look at the juicy error-handling part. In case of an error, LEAF will consider each of the `handler...` functions, [underline]#in order#, and call the first suitable match:
...
<6> This last handler is a catch-all for any error, in case no other handler could be selected: it prints diagnostic information to help debug logic errors in the program, since it failed to match an appropriate error handler to the error condition it encountered.

WARNING: It is critical to understand that the error handlers are considered in order, rather than by finding a "best match". No error handler is "better" than the others: LEAF will call the first one for which all of the arguments can be supplied using the available error objects. See <<handler_selection_procedure,handler selection procedure>> for details.
...
<5> List of error handlers goes here. We'll see that later.
...
TIP: The complete program from this tutorial is available https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result.cpp?ts=4[here]. The https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_eh.cpp?ts=4[other] version of the same program uses exception handling to report errors (see <<introduction-eh,below>>).
...
<2> If any of the functions we call throws, `try_catch` will find an appropriate handler to invoke. We'll look at that later.
...
TIP: The complete program from this tutorial is available https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_eh.cpp?ts=4[here]. The https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result.cpp?ts=4[other] version of the same program does not use exception handling to report errors (see the <<introduction-result,previous introduction>>).
...
With this in place, `file_read` from the https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result.cpp?ts=4[print_file_result.cpp] example can be rewritten like this:
...
* https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_result_error_tags.cpp?ts=4[print_file_result_error_tags.cpp] (using `leaf::result<T>`).
* https://github.com/zajo/leaf/blob/master/examples/print_file/print_file_eh_error_tags.cpp?ts=4[print_file_eh_error_tags.cpp] (using exception handling).
...
[[tutorial-working_with_std_error_code]]
...
The idea behind `std::error_code` is to encode both an integer value representing an error code, as well as the domain of that value. The domain is represented by a [underline]#reference#, `cat`, to a static object of type `std::error_category`. Conceptually, a `std::error_code` is like `pair<cat, int>`.

The point of wrapping an error code enum in a `std::error_code` is to erase its type, so it can travel freely across API boundaries. Let's say we have this `enum`:
...
We need some boilerplate to define our `std::error_category`:
...
std::error_code make_error_code(libfoo_error e) <3>
{
  return std::error_code(int(e), libfoo_error_category());
}
----

<1> Tell the standard library that `libfoo_error` is a designed to be used with `std::error_code`.
<2> Helper function to obtain a reference to the static `std::error_category` instance that describes `libfoo` errors.
<3> A factory function that converts a `libfoo_error` enum to a `std::error_code` object.
...
In terms of LEAF, we can write an error handler that matches `libfoo_error::e1` or `libfoo_error::e2` like this:

[source,c++]
----
[]( leaf::match<leaf::code<libfoo_error>, libfoo_error::e1, libfoo_error::e2> )
{
  // We got either a libfoo_error::e1 or a libfoo_error::e2
}
----

When we pass an instance of `leaf::code` as the first argument of the `leaf::match` template, it understands that the error object we want matched is not of type `libfoo_error`, but of type `std::error_code`. To check if the error handler is suitable to handle the received `std::error_code` object `ec`, LEAF checks if either of `ec == libfoo_error::e1` or `ec == libfoo_error::e2` is `true`.

So far so good, but remember, the standard library defines another type also, `std::error_condition`. The first confusing thing is that in terms of its physical representation, `std::error_condition` is identical to `std::error_code`; that is, it is also like a pair of `std::error_category` and an `int`!

So why do we need two different types which use identical physical representation?

The key to answering this question is to understand that `std::error_code` objects are designed to be returned from functions to indicate failures. In contrast, `std::error_condition` objects are [underline]#never# supposed to be communicated; their purpose is to provide an interpretation for the `std::error_code` values being communicated. The idea is that in a given program there may be multiple different "physical" (maybe platform-specific) `std::error_code` values which all indicate the same "logical" `std::error_condition`.

This leads us to the second confusing thing about `std::error_condition`: it uses the same `std::error_category` type, this time to specify what `std::error_code` values are equivalent to what `std::error_condition` values.

Let's say that our program uses two libraries, `libfoo` and `libbar`, each communicated in terms of `std::error_code` with different error categories. Let's say `libbar_error` looks like this:
...
// - make_error_code overload for libbar_error.
----
...
std::error_category const & libfoo_error_category() <3>
{
  struct cat: std::error_category
  {
    char const * name() const noexcept override
...
std::error_condition make_error_condition(my_error_condition e) <3>
{
...
<2> This specialization tells the standard library that the `my_error_condition` enum is designed to be used with `std::error_condition`.
<3> Define the `std::error_category` for `my_error_condition` `std::error_condition` objects.
<4> Here we specify that any of `libfoo:error::e1`, `libbar_error::e3` and `libbar_error::e4` are logically equivalent to `my_error_condition::c1`...
<5> ...and that any of `libfoo:error::e2`, `libbar_error::e1` and `libbar_error::e2` are logically equivalent to `my_error_condition::c2`.

Phew!

Now, if we have a `std::error_code` object `ec`, we can check if it represents `my_error_condition::c1` like so:
...
Finally, the equivalent LEAF error handler would look like this:

[source,c++]
----
[]( leaf::match<condition<my_error_condition>, my_error_condition::c1> )
{
  // We have a c1 in our hands
}
----

When we pass an instance of `leaf::condition` as the first argument of the `leaf::match` template, it understands that the error object we want matched is not of type `my_error_condition`, but of type `std::error_code`. To check if the error handler is suitable to handle the received `std::error_code` object `ec`, LEAF checks if `ec == my_error_condition::c1` is `true`.
...
Taking `my_info const &` means that the handler will only be selected if the caught exception object carries `my_info` (which LEAF accesses via `boost::get_error_info`). Alternatively, we can take `my_info const *`, in which case we'll get a `my_info` if it is available, or `0` if it is not.
...
See https://github.com/zajo/leaf/tree/master/examples[github].
...
. If that fails, they invoke the first of the specified error handlers that LEAF is able to supply with arguments using the available (in `ctx`) error objects.
...
namespace boost { namespace leaf {
  ...
  public:
    ...
    using enum_type = <<deduced>>;
    using matched_type = <<deduced>>;

    explicit match( matched_type const * m ) noexcept;

    explicit bool operator()() const noexcept;

    matched_type const & matched() const noexcept;
  };
...
[.text-right]
<<context.hpp>> | <<try_handle_all>> | <<try_handle_some>> | <<match>> | <<condition>> | <<code>> | <<error_info>> | <<diagnostic_info>> | <<verbose_diagnostic_info>>
====
...
** may take arguments, by value, of the predicate type `<<match,match>><E, V...>`, where `E` is enumerator or an instance of the <<condition>> class template.
** may take arguments, by value, of the predicate type `<<catch_,catch_>><Ex...>`, where each of the `Ex` types derives from `std::exception` (in this case, please also `#include <boost/leaf/handle_exception.hpp>`);
...
** If `A~i~` derives from `std::exception`, and the `try_block` throws a type that derives from `std::exception` which can be converted, using `dynamic_cast`, to `A~i~`, `a~i~` is initialized with a reference to the exception object;
...
<<result>> | <<e_file_name>>
+
<1> In case the `try_block` (the first lambda) indicates a failure, this handler will be selected if `ctx` stores an `e_file_name` associated with the error. Because this is the only supplied handler, if an `e_file_name` is not available, `try_handle_some` will return the `leaf::result<int>` returned by `f`.
<2> Print the file name, handle the error.
+
* If `a~i~` is of type `A~i~` `const pass:[*]` or `A~i~` `pass:[*]`, `try_handle_some` is always able to produce it: first it attempts to produce it as if it is taken by reference; if that fails, `a~i~` is initialized with `0`.
+
.Example:
[source,c++]
----
...
<<result>> | <<match>>
+
<1> This handler is selected if the error includes an object of type `errors` with value `ec1`.
<2> This handler is selected if the error includes an object of type `errors` regardless of its value.
+
In particular, the `E` type used to instantiate the `match` template may be an instance of the <<condition>> class template, which is used to match enumerated values of some type `En` for which `std::is_error_condition_enum<En>::value` is true:
+
.Example:
[source,c++]
----
...
+
<1> This handler is selected if the error includes an object of type `std::error_code` that compares equal to `make_error_condition(condx::x11)`.
<2> This handler is selected if the error includes any object of type `std::error_code`.
+
In particular, the `E` type used to instantiate the `match` template may be an instance of the <<code>> class template, which is used to match enumerated values of some type `En` for which `std::is_error_code_enum<En>::value` is true:
+
.Example:
[source,c++]
----
enum class my_error_code { e1=1, e2, e3 };

namespace std
{
  template <> struct is_error_code_enum<my_error_code>: true_type { };
};
...
try_handle_some(
  ...
  [&c](leaf::match<leaf::code<my_error_code>, my_error_code::e2>) <1>
  {
    ....
  },
...
<<result>> | <<match>> | <<code>>
+
<1> This handler is selected if the error includes an object of type `std::error_code` that compares equal to `make_error_code(my_error_code::e2)`.
<2> This handler is selected if the error includes any object of type `std::error_code`.
+
...
+
<1> This handler is selected if the current exception is either of type `exception1` or `exception2`.
<2> This handler matches any error. Use `info.<<error_info,exception_caught>>()` to check if `try_handle_some` has caught an exception, in which case you can call `info.exception()` to access it as `std::exception`.
+
CAUTION: Using <<catch_>> with `try_handle_some` requires `#include <boost/leaf/handle_exception.hpp>`.
+
TIP: If you want to `catch_` a single exception type `Ex` that derives from `std::exception`, using `catch_` is not required -- simply take an argument of type `Ex const &` or `Ex &`. The use of `catch_` is required only if you want to match any one of a list of exception types, or if `Ex` does not derive from `std::exception`.
+
...
NOTE: The `catch_` template is useful only as an argument to a handler function passed to a LEAF error-handling function, such as <<try_handle_all>>, <<try_handle_some>> or <<try_catch>>.
...
namespace boost { namespace leaf {
  ...
  struct code
  {
    ...
    static_assert(std::is_error_code_Enum<enum_type>::value);
  };

} }
----

NOTE: The `code` template is useful only as argument to the <<match>> template, to match a received `std::error_code` to a list of values of an error code enum.

.Example:
[source,c++]
----
enum class my_error_code_enum { e1=1, e2, e3 };

namespace std
{
  template <> struct is_error_code_enum<my_error_code_enum>: true_type { };
};

std::error_code ec;
match<code<my_error_code_enum>, my_error_code_enum::e1> m(&ec);

// m() evaluates to true iff ec == make_error_code(my_error_code_enum::e1).
----
...
e_wrapped_error_code ec;
match<code<e_wrapped_error_code, my_error_code_enum>, my_error_code_enum::e1> m(&ec);

// m() evaluates to true iff ec.value == make_error_code(my_error_code_enum::e1).
----
...
namespace boost { namespace leaf {
  ...
  struct condition
  {
    ...
    static_assert(std::is_error_condition_Enum<enum_type>::value);
  };

} }
----

NOTE: The `condition` template is useful only as argument to the <<match>> template, to match a received `std::error_code` to a list of values of an error condition enum.
...
// m() evaluates to true iff ec == make_error_condition(cond_x::x11).
----
...
e_wrapped_error_code ec;
match<condition<e_wrapped_error_code, cond_x>, cond_x::x11> m(&ec);

// m() evaluates to true iff ec.value == make_error_condition(cond_x::x11).
----
...
This class template is used only as an argument to an error handler. The <<handler_selection_procedure,handler selection procedure>> uses the boolean conversion operator to determine if the error object under consideration matches one of the specified `V...` values.

The type of the `V...` parameter pack is the same as the deduced member `enum_type`. The type deductions and the behavior of the boolean conversion operator follow these rules:

* If `E` is an instance of the <<condition>> template:
** `match<E>::enum_type` is deduced as `typename E::enum_type`;
** `match<E>::matched_type` is deduced as `std::error_code`;
** The boolean conversion operator evaluates to `true` iff the `std::error_code` pointer `m` passed to the constructor is not `0` and the object it points compares equal to at least one of `make_error_condition(V)...`.
* Otherwise, if `E` is an instance of the <<code>> template:
** `match<E>::enum_type` is deduced as `typename E::enum_type`;
** `match<E>::matched_type` is deduced as `std::error_code`;
** The boolean conversion operator evaluates to `true` iff the `std::error_code` pointer `m` passed to the constructor is not `0` and the object it points compares equal to at least one of `make_error_code(V)...`.
* Otherwise, if `E` defines an accessible data member `value`:
** `match<E>::enum_type` is deduced as `decltype(E::value)`;
** `match<E>::matched_type` is deduced as `E`;
** The boolean conversion operator evaluates to `true` iff the `E` pointer `m` passed to the constructor is not `0` and `mpass:[->]value` is equal to one of `V...`.
* Otherwise, if `E` defines an accessible member function `value()`:
** `match<E>::enum_type` is deduced as `typename std::remove_reference<decltype(std::declval<E>().value()))>::type`;
** `match<E>::matched_type` is deduced as `E`;
** The boolean conversion operator evaluates to `true` iff the `E` pointer `m` passed to the constructor is not `0` and `mpass:[->]value()` is equal to one of `V...`.
* Otherwise:
** `match<E>::enum_type` is deduced as `E`;
** `match<E>::matched_type` is deduced as `E`;
** The boolean conversion operator evaluates to `true` iff the `E` pointer `m` passed to the constructor is not `0` and `*m` is equal to one of `V...`.

In all cases, if the boolean conversion operator evaluates to `true`, it is valid to call `matched` to obtain a reference to the matched object.
...
assert(!m2()); // 0 passed to the constructor

match<my_error_code, 42> m3(&ec);
assert(m3()); // 42==42
assert(&m3.matched() == &ec);

match<my_error_code, 1, 5, 42, 7> m4(&ec);
assert(m4()); // 42==1 || 42==5 || 42==42 || 42==7
assert(&m4.matched() == &ec);

match<my_error_code, 1, 3, -42> m5(&ec);
assert(!m5()); // !(42==1 || 42==3 || 42==-42 || 42==7)
----
...
match<my_error_code, e2> m3(&ec);
assert(m3()); // e2==e2
assert(&m3.matched() == &ec);

match<my_error_code, e1, e2> m4(&ec);
assert(m4()); // e2==e1 || e2==e2
assert(&m4.matched() == &ec);

match<my_error_code, e1, e3> m4(&ec);
assert(!m4()); // !(e2==e1 || e2==e3)
----
