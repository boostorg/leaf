...
Stated differently, the problem is that attempts to harness the {CPP} static type checking system to help enforce correct error handling based on the types of errors a function may return leads (in generic contexts) to dynamic error types. Then, yes, it is possible to turn every error *type* into, for example, a `std::error_code` *object*, but this immediately renders meaningless the attempt to use static type checking to ensure that all error conditions are handled. Worse, the coupling between the signatures of <<rationale,error-neutral>> functions (read: most functions in a program) and the error types they need to forward to their callers interferes with the lossless propagation of errors throughout the program.

Conclusion: :: Error handling, by its very nature, is a dynamic process. An effective error handling strategy is to:
+
* Facilitate efficient lossless propagation of error objects of arbitrary types throughout the program, independent of funciton signatures (LEAF).
* Write unit tests for each and every error condition. To this admittedly difficult end, unfortunately, static type checking can only provide limited assistance.
