...
Instead of the https://www.boost.org/doc/libs/release/libs/exception/doc/get_error_info.html[`boost::get_error_info`] API defined by Boost Exception, it is possible to use LEAF error handlers directly (this requires that the LEAF header `boost_exception_support.hpp` is included).

Consider the following use of `boost::get_error_info`:
...
[[boost_exception_support.hpp]]
==== `boost_exception_support.hpp`

Including this header enables LEAF error handlers to access Boost Exception error information. See <<tutorial-boost_exception_integration>>.
