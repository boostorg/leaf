...
NOTE: This part of the introduction is about using LEAF without exception handling. LEAF works great <<introduction-eh>> as well.
...
Handler: :: A function (almost always a lambda), which is able to handle a specific error condition identified by its arguments (usually of E-types). In typical use, if a low-level function attempts to communicate an E-object, it is discarded unless at least one error-handling scope up the call chain contains a handler that takes an argument of that E-type.
+
Scopes that handle errors require an `error_id` and a list of handlers, which they typically pass to `try_handle_some`, `try_handle_all` or `try_catch`. The `error_id` is transported in one of the following ways:
+
--
* in a <<result,`leaf::result<T>`>> object,
* in a `std::error_code` object, or
* in an exception object returned by <<exception,`leaf::exception`>>.
--
+
...
<1> This handler is selected based on the static type of `ec`. If another error is reported, it will be returned to the caller.
...
[source,c++]
----
struct io_error: virtual std::exception { };
...
Using LEAF, we can classify errors without having to organize them in a hierarchy. We could simply say:
...
[source,c++]
----
void read_file( FILE * f, void * buffer, size_t buffer_size )
{
  ....
  if( error )
    throw leaf::exception(io_error{}, read_error{}, file_error{});
}
----
[.text-right]
<<exception>>

The above `throw` statement will throw a `std::exception`, and will associate the passed objects with it, which now act as a simple dynamic tagging mechanism to help classify the error.

The error handlers can simply pick and choose what kind of errors they want to handle:
...
<1> The try block calls functions which throw `std::exception` objects, associating additional objects with them using `leaf::exception`. The error handlers that follow are processed in order.
<2> If the error has been classified as both `file_error` and `read_error` (that is, it is a file read error), it'll be handled here.
<3> Else, if the error has been classified as `read_error`, it will be handled here.
...
Requirements: :: Each of `f~i~` in `f...` must be a function that does not throw exceptions and takes a single argument of type `E~i~` such that `E~i~` defines an accessible no-throw default constructor.
...
Requirements: :: Each of `f~i~` in `f...` must be a function that does not throw exceptions, takes no arguments and returns an object of a no-throw movable type `E~i~`.
...
namespace boost { namespace leaf {
  ...
  template <class Ex, class... E>
  <<unspecified>> exception( Ex && ex, E && ... e ) noexcept; <1>

  template <class... E>
  <<unspecified>> exception( E && ... e ) noexcept; <2>

} }
----
The `exception` function provides two kinds of overloads, selected automatically based on the type of the first argument:

<1> Selected if the first argument is an exception object, that is, `Ex` derives publicly from `std::exception`. In this case the return value is of unspecified type which derives publicly from `Ex` *and* from class <<error_id>>, such that:
...
Requirements: :: `E` must be no-throw movable.
