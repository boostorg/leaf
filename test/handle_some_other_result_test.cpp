...
int main()
{
	...
	}
	{
		result<int,std::error_code> r = leaf::handle_some(
			...
			[ ]( info<42> const & x, leaf::match<leaf::condition<std::errc>, std::errc::no_such_file_or_directory> ec )
			{
				BOOST_TEST(x.value==42);
				return ec.value();
			} );
		BOOST_TEST(!r);
		BOOST_TEST(r.error() == make_error_code(std::errc::no_such_file_or_directory));
	}
