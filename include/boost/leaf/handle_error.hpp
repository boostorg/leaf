...
namespace boost { namespace leaf {
	...
	namespace leaf_detail
	{
		...
		LEAF_ALWAYS_INLINE typename std::decay<decltype(std::declval<TryBlock>()().value())>::type nocatch_context<E...>::try_handle_all( TryBlock && try_block, H && ... h )
		{
		...
		LEAF_ALWAYS_INLINE typename std::decay<decltype(std::declval<TryBlock>()().value())>::type nocatch_context<E...>::remote_try_handle_all( TryBlock && try_block, RemoteH && h )
		{
			auto active_context = activate_context(*this, on_deactivation::do_not_propagate);
		...
		template <class... E>
		template <class TryBlock, class... H>
		LEAF_ALWAYS_INLINE typename std::decay<decltype(std::declval<TryBlock>()())>::type nocatch_context<E...>::try_handle_some( TryBlock && try_block, H && ... h )
		{
		...
		template <class... E>
		template <class TryBlock, class RemoteH>
		LEAF_ALWAYS_INLINE typename std::decay<decltype(std::declval<TryBlock>()())>::type nocatch_context<E...>::remote_try_handle_some( TryBlock && try_block, RemoteH && h )
		{
			auto active_context = activate_context(*this, on_deactivation::propagate_if_uncaught_exception);
