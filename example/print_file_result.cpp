...
leaf::result<std::shared_ptr<FILE>> file_open( char const * file_name )
{
	if( FILE * f = fopen(file_name,"rb") )
		...
		return leaf::error( error_code::input_file_open_error, e_file_name{file_name}, e_errno{errno} );
}
...
leaf::result<int> file_size( FILE & f )
{
	...
	if( fseek(&f,0,SEEK_END) )
		return leaf::error( error_code::input_file_size_error );
...
leaf::result<void> file_read( FILE & f, void * buf, int size )
{
	...
	if( ferror(&f) )
		return leaf::error( error_code::input_file_read_error, e_errno{errno} );

	if( n!=size )
		return leaf::error( error_code::input_eof_error );
...
leaf::result<void> print_file( char const * file_name )
{
	...
	std::cout.flush();
	if( std::cout.fail() )
		return leaf::error( error_code::cout_error );
...
int main( int argc, char const * argv[ ] )
{
	...
	else
	{
		...
		{
			case error_code::input_file_open_error:
			{
			...
			case error_code::input_file_size_error:
			case error_code::input_file_read_error:
			case error_code::input_eof_error:
			{
				...
				bool matched = handle_error( exp, r,
					...
					[ ] ( e_file_name const & fn, e_errno const & errn )
					{
						std::cerr << "Failed to access " << fn.value << ", errno=" << errn.value << std::endl;
					},

					[ ] ( e_errno const & errn )
					{
						std::cerr << "I/O error, errno=" << errn.value << std::endl;
					},
