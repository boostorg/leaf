...
Let's assume that to handle errors from a given function `f` we need an error code and a file name. In addition -- depending on the error -- we may need a request ID, also a string. In terms of LEAF, we can write:
...
<3> The call to `f()` has failed; `handle_error` takes `exp`, `r`, and any number of error handling functions to be matched, in order, against error objects available in `exp`, associated with `r`.
...
Because both the file name and the request ID are strings, we will wrap each of them in a simple `struct`, so we can tell them apart:
...
Let's assume that to handle exceptions from a given function `f` we need a file name. In addition -- depending on the error -- we may need a request ID, also a string. In terms of LEAF, we can write:
...
<2> We caught a `my_error` exception; `handle_error` takes `exp`, `e`, and any number of error handling functions to be matched, in order, against error objects available in `exp`, associated with `e`.
...
With LEAF, users can efficiently associate with errors or with exceptions any number of values that pertain to a failure. These values may be of arbitrary movable types.

However, when using types that are not unique to error handling, each value should be enclosed in a C-`struct`, which acts as its compile-time identifier and gives it semantic meaning. Examples:
...
By convention these structs use the `e_` prefix and define a single data member called `value`, whough any `noexcept`-movable type may be used.
...
int do_work( lua_State * L ) noexcept
{
  ...
  {
    leaf::peek_next_error().propagate(ec1); //<3>
